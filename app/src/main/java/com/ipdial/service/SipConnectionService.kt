                ...
                withContext(Dispatchers.Main) {
                    if (success) {
                        ...
                        if (session != null) {
                            ...
                            // A PJSIP state callback may have arrived before Telecom finished
                            // creating this Connection. Reconcile immediately so EARLY/CONFIRMED
                            // is not lost during the callback-to-Connection registration window.
                            when (session.state) {
                                CallState.CONFIRMED -> connection.setActive()
                                CallState.EARLY -> connection.setRinging()
                                CallState.CONNECTING, CallState.CALLING -> connection.setDialing()
                                CallState.DISCONNECTED -> {
                                    disconnectCall(session.callId, DisconnectCause.REMOTE)
                                }
                                else -> connection.setDialing()
                            }
                            com.ipdial.util.SipLogger.log(TAG, "Connection registered successfully with callId=${session.callId}, reconciledState=${session.state}")
                        } else {
