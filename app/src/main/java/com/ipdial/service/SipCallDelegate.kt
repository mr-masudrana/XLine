...
class SipCallDelegate(
    ...
    override fun onCallState(prm: OnCallStateParam) {
        ...
        // PJSIP may invoke onCallState() before makeCall() returns and before
        // SipEngine has had a chance to put this delegate into callMap. Register
        // the native call immediately to close that callback/Telecom race window.
        val existing = callMap.putIfAbsent(currentCallId, this)
        if (existing == null) {
            log("ONCALLSTATE RACE FIX: early callback registered callId=$currentCallId", false)
        }

        // The first outgoing callback can arrive while the session still has callId=-1.
        // Promote it here so ConnectionService and UI always see the real native id.
        val sessionBefore = _callSession.value
        if (sessionBefore != null && sessionBefore.callId == -1) {
            _callSession.value = sessionBefore.copy(callId = currentCallId)
        }
